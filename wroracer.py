...
time_ditch_banka = 10
global_speed = 140 # Устанавливаем скорость робота 130
povorot_speed = 90
...
if flag_qualification: 
    global_speed = 180 # Добавляем скорости на квалификации 240 в оражневую, 220 в синею
    povorot_speed = 180 # Скорость поворота в квалификации 180
...
robot = rapi.RobotAPI(flag_pyboard=True, flag_send_video=flag_video) # инициализация робота
...
def Find_black_line(frame, frame_show, direction, flag_draw=True):
    ...
    Returns:
        int: Y чёрной линии.
    """
    x1, y1 = 640 - 20, 220 # Задаём 1-е координаты области определения чёрной линии слева.
    x2, y2 = 640, 320 # Задаём 2-е координаты области определения чёрной линии слева.

    if direction == 1: 
        x1, y1 = 0, 220 # Задаём 1-е координаты области определения чёрной линии справа.
        x2, y2 = 20, 320 # Задаём 1-е координаты области определения чёрной линии справа.
...
def Find_box(frame, frame_show, color, flag_draw=True):
    ...
    if contours:
        ...
        if flag_draw:
            ...
            cv2.putText(frame_show, str(round(area, 1)) + str(x >= 150 and x <= 400), (x + x1, y - 40 + y1),
        ...
        return x + w / 2, y + h, area, x >= 150 and x <= 400
...
def go_back(angle, time1, time2):
    global timer_finish
    robot.serv(angle)
    robot.move(-200, 0, time1, wait=False)
    robot.wait(time1)
    ...
    if state == "Manual move":
        ...
        if direction == 1:
            ...
            if not flag_qualification:
                ...
                # cord_red_banka, area_red_banka, d = Find_box(frame, frame_show, "red_up")
                # if area_red_banka is not None:
                #     delta_banka = delta_red_plus
                ...
                # cord_green_banka, area_green_banka, d = Find_box(frame, frame_show, "green")
                # if area_green_banka is not None:
                #     delta_banka = delta_green_plus
                #     timer_banka = time.time()
                ...
                if area_green_banka is not None:
                    # print(x_green_banka, y_green_banka, area_green_banka, d)
                    delta_green_plus = 0
                    if x_green_banka < 500: # + 150 - y_green_banka
                        delta_green_plus = (500 - x_green_banka)/6
                    if y_green_banka > 50:
                        delta_green_plus += (y_green_banka - 50)/5
                    delta_banka = delta_green_plus
                    if direction == -1:
                        delta_banka /= 1.5
                    if y_green_banka > 220 and x_green_banka < 480:
                        go_back(-35, time_go_back_banka, time_go_banka)

                x_red_banka, y_red_banka, area_red_banka, d = Find_box(frame, frame_show, "red_up")
                if area_red_banka is not None:
                    delta_red_plus = 0
                    if x_red_banka > 100: # - 150 + y_red_banka 
                        delta_red_plus = (x_red_banka - 100)/6
                    if y_red_banka > 50:
                        delta_red_plus += (y_red_banka - 50)/5
                    delta_banka = -delta_red_plus
                    if direction == 1:
                        delta_banka /= 1.5

                    # delta_banka = -delta_banka
                    if y_red_banka > 220 and x_red_banka > 130:
                        go_back(35, time_go_back_banka, time_go_banka)
        ...
        p = regulate(max_y2, max_y)*direction
        ...
        if not flag_qualification:
            ...
            robot.serv(p)
        robot.move(peed, 0, 100, wait=False)
