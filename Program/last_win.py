...
porog_black_line_minus = 290  #Black
# porog_black_line_minus = 285
# porog_black_line_plus = 300 #Lime
porog_black_line_plus = 290 #Black
...
if flag_qualification:
    ...
    global_speed += 65.3
    pause_finish = 0.38
    porog_black_line_plus+=25  # по часовой стрелке 40
    porog_black_line_minus+=25   # против 40
...
delta_green_minus = 12 #8
delta_red_minus = -12
time_go_back_banka = 450 # 415
...
pause_povorot = 1 #0.45 SEC 0.45 sec the qula

if flag_qualification:
    pause_povorot = 0.45  # 0.45 SEC 0.45 sec the dolgaia qula
...
while True:
    ...
    if state == "Manual move":
        ...
        if time.time() < timer_state + pause_povorot:
            # print(pause_povorot)
            print(pause_povorot, timer_state)
            time.sleep(0.02)
