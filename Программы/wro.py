...
time_ditch_banka = 10
global_speed = 110 # Устанавливаем скорость робота 130
low_speed = 110
before_line_speed = 110
povorot_speed = 80
minspeed = 110
pause_finish = 2 # Устанавливаем пауза перед остановкой на финише 1.25
porog_black_line_minus = 140 # Устанавливаем порог чёрной линии по часовой стрелке 305
...
if flag_qualification:  
    povorot_delay = 0.5
    low_speed = 60
    before_line_speed = 60
    global_speed = 70 # Добавляем скорости на квалификации 240 в оражневую, 220 в синею
...
banka_timer = 0
left_pix = 0
last_forward_event_received = 0
last_speed = 0

robot = GPIORobotApi(acceleration_enabled=False, invert_servo=True) # инициализация робота
...
def Find_stop(frame, frame_show, direction, flag_draw=True, mode=0):
    ...
    x1_1, y1_1 = 340+10, 340
    x2_1, y2_1 = 360+10, 370

    if mode == 1:
        x1, y1 = 280-20, 340
        x2, y2 = 300-20, 370

        x1_1, y1_1 = 340+20, 340
        x2_1, y2_1 = 360+20, 370
...
def put_telemetry(frame_show):
    ...
    global fps, fps_last, fps_timer, left_pix
...
def go_back(angle, time1, time2):
    global timer_finish
    robot.serv(GK+angle)
    robot.move(150, COUNTERCLOCKWISE)
    robot.wait(time1)
...
while True:
    ...
    if time.time() - fps_timer >= 1:
        ...
        fps = 0
        fps_timer = time.time()
    if time.time() - last_forward_event_received > 0.2:
        robot.move(0)
    fps += 1
    if state == "Manual move":
        ...
        if k == 32:
            ...
            robot.serv(GK+manual_serv)
        if k == 38:
            last_forward_event_received = time.time()
            robot.move(speed_manual)
        if k == 40:
            robot.move(speed_manual, COUNTERCLOCKWISE)
        if k == 66:
            robot.beep()
            print('b')
        if k == 82:
        ...
        if flag_qualification:
            ...
            robot.serv(GK+0)
        robot.move(abs(before_line_speed), before_line_speed >= 0)
        ...
        if time.time() < timer_state + pause_povorot:
            # speed = minspeed if Find_stop(frame, frame_show, direction, mode=1 if flag_qualification else 0) else low_speed
            # fs = Find_stop(frame, frame_show, direction, mode=1 if flag_qualification else 0)
            # speed = minspeed if fs else low_speed
            # if count_lines >= 5:
        ...
        if count_lines >= max_lines:
            ...
            if count_lines > max_lines:
                robot.beep()
                robot.serv(GK+0)
                robot.move(255, COUNTERCLOCKWISE)
                robot.tick()
                time.sleep(3)
                robot.move(0)
                robot.tick()
        ...
        if flag_qualification:
            ...
            ff = Find_stop(frame, frame_show, direction, mode=1)
        speed = minspeed if ff else global_speed
        if abs(max_y2 - max_y) < 10:
            p = 0
        else:
            p = reg_move.apply(max_y2, max_y)
        p = -60*direction if ff else p * direction
        ...
        if delta_banka != 0 and not flag_qualification:
            ...
            after_banka_timer = True

        if delta_banka != 0 and not flag_qualification:
            robot.serv(GK+delta_banka)
        else:
            if after_banka_timer is True:
                after_banka_timer = time.time() + after_banka_delay
            if type(after_banka_timer) == float and time.time() <= after_banka_timer:
                robot.serv(GK+constraint(p+k-last_banka_delta, -50, 50))
            else:
                robot.serv(GK+p+k)
