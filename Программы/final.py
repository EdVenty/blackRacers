...
black_line_porog_clockwise = 140 # Устанавливаем порог чёрной линии по часовой стрелке 305
black_line_porog_counterclockwise = 140 # Устанавливаем порог чёрной линии по часовой стрелке 305
...
pause_povorot = 0.2 # Устанавливаем максимальную задуржку на повороте
...
robot = RobotAdapter(acceleration_enabled=False, invert_servo=True)
...
def Find_stop(frame, frame_show, direction, flag_draw=True, mode=0):
    ...
    x1_1, y1_1 = 340+100, 320
    x2_1, y2_1 = 360+100, 480

    if mode == 1:
        x1, y1 = 280-100, 280
        x2, y2 = 300-100, 480

        x1_1, y1_1 = 340+100, 280
        x2_1, y2_1 = 360+100, 480
...
def Find_box(frame, frame_show, color, mode=0, flag_draw=True):
    ...
    _, contours, hierarchy = cv2.findContours(
        mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    # перебираем все найденные контуры
    contours = [i for i in contours if cv2.contourArea(i) > 700]
    if contours:
...
def put_telemetry(frame_show):
    ...
    cv2.putText(frame_show, "Frag: " + str(fragments), (10, 140), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
                (255, 255, 255), 2)
    cv2.putText(frame_show, "Speed: " + speed_type + ' ' + str(global_speed), (10, 160), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
                (255, 255, 255), 2)
    ...
    if direction == 0:
        ...
        if not flag_qualification:
            ...
            if area_green_banka is not None:
                ...
                delta_green_plus = 0
                # if x_green_banka < 550:
                delta_green_plus = (600 - x_green_banka)/30
                if y_green_banka >= 160:
    ...
    if direction == -1:
        if not flag_qualification:
            ...
            if area_green_banka is not None:
                ...
                delta_green_plus = 0
                # if x_green_banka < 550:
                delta_green_plus = (640 - x_green_banka)/40
                if y_green_banka >= 200:
                    delta_green_plus *= 1.5
                delta_banka = delta_green_plus
...
while True:
    ...
    if state == "Manual move":
        ...
        if flag_qualification:
            ...
            ff = Find_stop(frame, frame_show, direction, mode=1)
            fff = Find_stop(frame, frame_show, -direction, mode=0)
        speed = global_speed if (ff or fff) else global_speed

        # if abs(max_y2 - max_y) < 10:
        #     p = 0
        # else:
        p = reg_move.apply(max_y2, max_y)
        p = -120*direction if (ff or fff) else p * direction
        if fff:
            p *= -1
